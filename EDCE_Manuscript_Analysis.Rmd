---
title: "Cold Exposure Induces Dynamic, Heterogeneous Alterations in Human Brown Adipose Tissue Lipid Content: Reproducible Analysis"
...
### Standard Deviation of the Difference for Multiple Variables
```{r sdfun, results='hide'}
sdfun <- function(outcome, group) {
  SDdif <- -diff(tapply(outcome, group, sd, na.rm=TRUE))
  SDdif <- round(SDdif[1],2)
}
```
...
#Labels for FSF Decades - Short and Long Format
...
#FSF Decade Names for Facet Plots
...
#save_plot("Fig3.eps", Fig3, device=cairo_ps, base_width=15, base_aspect_ratio = 1.618, base_height=NULL)
```
...
Import fat-water MRI data and separate into ROI and FSF Decade data frames. Identify the data points associated with the thermoneutral ("TN") condition (i.e. the initial image acquisition) and the end of the cooling protocol ("CE") using the final image acquisition achieved in all participants. 
```{r importMRI, include=FALSE}
# Import MRI Data
...
#WAT Decade Data - Replace Scan # with TN or CE Flag
TnCeWAT<-WATDec.Data %>% dplyr::filter(nScan==4|nScan==24) %>% mutate(nScan=factor(ifelse(nScan == 4,"TN","CE")))

#MUS Decade Data - Replace Scan # with TN or CE Flag
...
## Standard Deviation of Difference - TN and CE
# BAT Decades
TnCeDec <- TnCeDec %>% group_by(ROIName) %>% mutate(SDDiff=sdfun(Mean,nScan))

# WAT Decades
TnCeWAT <- TnCeWAT %>% group_by(ROIName) %>% mutate(SDDiff=sdfun(Mean,nScan))

# MUS Decades
TnCeMUS <- TnCeMUS %>% group_by(ROIName) %>% mutate(SDDiff=sdfun(Mean,nScan))

# ROI Data
TnCeRoi <- TnCeRoi %>% group_by(ROIName) %>% mutate(SDDiff=sdfun(Mean,nScan))
...
#save_plot("Fig4.eps", Fig4, device=cairo_ps, base_width=12, base_aspect_ratio = 1.618, base_height=NULL)  #Save to svg for transparency
```


## Figure 5: Effect of Cold Exposure on Mean Fat-Signal Fraction in each Tissue Decade
Comparison of thermoneutral and cold exposure mean fat-signal fractions (%) for Decade group assignments in brown adipose tissue (BAT;**A**), muscle (MUS; **B**), and subcutaneous adipose tissue (SAT;**C**). Image voxels included in each tissue region of interest were assigned to a FSF decade according to the voxel's initial fat-signal fraction value (i.e. thermoneutral). For example, a fat-signal fraction value of 33% was assigned to the 30% decade. A minimum of 60 voxels were required for the decade to be included in the summary analysis. The center line in each box indicates the mean, and the top and bottom of the box show the 95% bootstrapped confidence intervals (1000 samples). Wilcoxon signed rank tests were performed to assess statistical comparisons: *_P_<0.05.
```{r Fig5, echo=FALSE}
...
#save_plot("Fig6.eps", Fig6, device=cairo_ps, base_width=12, base_aspect_ratio = 1.618, base_height=NULL)  #Save to svg for transparency
```
...
#save_plot("Fig7.eps", Fig7, device=cairo_ps, base_width=12, base_aspect_ratio = 1.618, base_height=NULL)  #Save to svg for transparency
```


### Spearman Rank Correlation: Mean FSF vs. Thermal Sensation
Correlation (ρ) between mean fat-signal fraction (FSF) and thermal sensation for each Decade group was evaluated with the Spearman rank test. Note, correlation values are inverted from the displayed plot due to the reverse scale position (Neutral = 50, V. Cold = 0).
```{r spearmanDec}
...
Plot the effect of temperature change on the quantification of FSF at each of the BAT FSF Decades.  
```{r FigS1, echo=FALSE}
...
FigS1<-Sim.Long %>% ggplot(aes(x=Temperature, y=(Bias*100), color=FSF)) +
  geom_line(size=0.8) + 
...
#save_plot("FigS1.eps", FigS1, device=cairo_ps, base_width=12, base_aspect_ratio = 1.618, base_height=NULL)  #Save to svg for transparency
```